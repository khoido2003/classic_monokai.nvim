...
M.extras = {
  {
    name = "palettes",
    ext = ".lua",
    label = "Monokai Palettes",
    both_styles = true,
    url = "https://github.com/polirritmico/monokai.nvim/tree/main/extras/palettes",
  },
  {
    name = "kitty",
    ext = ".conf",
...
function M.reset_monokai()
  for pkg in pairs(package.loaded) do
    if pkg:find("^monokai") then
  ...
  return require("monokai.config").extend()
end

---Get the dark and light default palettes
---@return {dark:ColorScheme, light:ColorScheme}
function M.default_colors()
  local defaults = M.reset_monokai()
  defaults.style = "dark"
  local colors_dark = require("monokai.colors").setup(defaults)
  defaults.style = "light"
  local colors_light = require("monokai.colors").setup(defaults)
...
function M.generate_extra_files(silent)
  ...
  local base_url = "https://github.com/polirritmico/monokai.nvim/raw/main/extras/"

  ---@param info monokai.ExtraInfo
  ---@param style string
  local function generate_extra_file(info, style)
    local colors = palettes[style]
    local filename = info.name .. "/monokai_" .. style .. info.ext
    local target_dir = base_dir .. filename
    local extra_mod = utils.mod("monokai.extras." .. info.name)
    colors["_upstream_url"] = base_url .. filename
    colors["_style_name"] = "Monokai " .. style:gsub("^%l", string.upper)
...
--- Run this function through require("monokai.extras").setup() to
