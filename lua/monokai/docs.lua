...
function M.highlight_mod_url(mod_plugin)
  local module = utils.mod("monokai.highlights." .. mod_plugin)
  return module and module.url or ""
end

function M.generate_cfg_spec_block()
  local cfg_path = vim.fn.fnamemodify(utils.me, ":p:h") .. "/monokai/config.lua"
...
function M.generate_implemented_plugins()
  local highlights = utils.mod("monokai.highlights")
...
function M.generate_extras_list()
  local extras = utils.mod("monokai.extras.init")
  local extras_list = {}
