...
function M.generate_kinds(hl, pattern)
  hl = hl or {}
  local base_kinds = require("monokai.highlights.kinds").base_kinds
...
function M.import_color(palette, color)
  local colors = M.mod("monokai.colors." .. palette)
...
function M.running_from_dev()
  ...
  local monokai_path = require("monokai.utils").me
...
function M.cache.file(filename)
  local base_path = vim.fn.stdpath("cache") .. "/monokai/dump-"
